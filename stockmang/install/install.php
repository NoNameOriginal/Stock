...
if (is_file($installFile)) {
    ...
    switch ($step) {
        ...
        if (phpversion() < '5.6') {
            $error = true;
            echo "<div class='alert alert-error'><i class='icon-remove'></i> Your PHP version is " . phpversion() . '! PHP 5.6 or higher required! (Recommended PHP7.1)</div>';
        } else {
        ...
        if ($_POST) {
            ...
            <form action="index.php?step=0" method="POST" class="form-horizontal">
                <div class="control-group">
                    <label class="control-label" for="username">Envato Username</label>
                    <div class="controls">
                        <input id="username" type="text" name="username" class="input-large" required data-error="Username is required" value="nulled" placeholder="Envato Username" />
                    </div>
                </div>
                <div class="control-group">
                    <label class="control-label" for="code">Purchase Code <a href="#myModal" role="button" data-toggle="modal"><i class="icon-question-sign"></i></a></label>
                    <div class="controls">
                        <input id="code" type="text" name="code" class="input-large" required data-error="Purchase Code is required" value="nulled" placeholder="Purchase Code" />
                    </div>
                </div>
        ...
        if ($_POST) {
			...
			$object->status = 'success';
            if ($object->status == 'success') {
				$sql = file_get_contents('smaall.sql');
                $dbdata = [
